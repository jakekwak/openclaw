...
- 기본적으로 OpenClaw는 개인 에이전트입니다: 단일 신뢰 운영자 경계.
- 공유/다중 사용자 구성은 잠금 구성이 필요합니다 (신뢰 경계 분리, 최소 도구 접근 유지, [보안](/ko-KR/gateway/security) 준수).
- 로컬 온보딩은 이제 새 구성의 기본값을 `tools.profile: "coding"`으로 설정합니다. 따라서 새 로컬 구성은 무제한 `full` 프로파일을 강제하지 않으면서도 파일시스템/런타임 도구를 유지합니다.
- hooks/webhooks 또는 기타 비신뢰 콘텐츠 피드를 활성화한다면, 강력한 최신 모델 티어를 사용하고 엄격한 도구 정책/샌드박싱을 유지하세요.
</Step>
...
- **이 Mac (로컬 전용):** 온보딩이 인증을 구성하고 로컬에 자격 증명을 기록할 수 있습니다.
- **원격 (SSH/Tailnet를 통해):** 온보딩은 로컬 인증을 구성하지 않습니다. 자격 증명은 게이트웨이 호스트에 있어야 합니다.
