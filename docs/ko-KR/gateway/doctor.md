...
- Doctor는 `models.providers.opencode`뿐 아니라 `models.providers.opencode-go` 오버라이드도 경고합니다.
- 레거시 cron 저장소(`jobId`, `schedule.cron`, 구형 payload/delivery 필드, `notify: true`) 마이그레이션도 점검합니다.
