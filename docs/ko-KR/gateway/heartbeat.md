...
5. 선택 사항: heartbeat가 `HEARTBEAT.md`만 필요하다면 lightweight bootstrap context를 사용합니다.
6. 선택 사항: 활동 시간을 로컬 시간으로 제한합니다.
  ...
  agents: {
    defaults: {
      heartbeat: {
        ...
        directPolicy: "allow", // 기본값: direct/DM 대상 허용; 억제하려면 "block"
        lightContext: true, // 선택 사항: bootstrap 파일 중 HEARTBEAT.md만 주입
...
- `lightContext`: true이면 heartbeat 실행이 lightweight bootstrap context를 사용하며, workspace bootstrap 파일 중 `HEARTBEAT.md`만 유지합니다.
...
- `directPolicy`: direct/DM 전달 동작 제어
  - `allow` (기본값): direct/DM 대상 전달 허용
  - `block`: direct/DM 대상 전달 억제 (`reason=dm-blocked`)
...
- 하트비트는 기본적으로 direct/DM 대상으로도 전달됩니다. direct 대상 전달을 막고 실행만 유지하려면 `directPolicy: "block"`을 설정하세요.
