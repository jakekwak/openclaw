...
- Exec approval은 exact request context와 best-effort direct local file operand에 바인딩되며, 모든 interpreter/runtime loader 경로를 완전하게 모델링하지는 않습니다.
- `channels.telegram.tokenFile`은 일반 파일만 허용하며 심볼릭 링크는 거부됩니다.
- `gateway.controlUi.allowInsecureAuth`는 localhost 비보안 HTTP 호환성 토글일 뿐입니다.
- 로컬 폴백은 `gateway.auth.*`가 비어 있을 때만 허용되며, SecretRef가 해석되지 않으면 fail-closed 됩니다.
