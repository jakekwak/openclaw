...
프로바이더 설정 예제 (OpenCode 포함)는 [/gateway/configuration](/gateway/configuration#opencode)에 있습니다.
...
`models.providers`에 있는 커스텀 프로바이더는 에이전트 디렉토리 (기본 `~/.openclaw/agents/<agentId>/agent/models.json`)의 `models.json`에 작성됩니다. 이 파일은 기본적으로 병합되며, `models.mode`가 `replace`로 설정된 경우를 제외합니다.

일치하는 프로바이더 ID에 대한 merge mode 우선순위:

- 에이전트 `models.json`에 이미 있는 비어 있지 않은 `baseUrl`이 우선합니다.
- 에이전트 `models.json`의 비어 있지 않은 `apiKey`는, 현재 config/auth-profile 문맥에서 해당 프로바이더가 SecretRef-managed가 아닐 때만 우선합니다.
- SecretRef-managed 프로바이더 `apiKey` 값은 해석된 비밀을 저장하는 대신 source marker(`ENV_VAR_NAME` 또는 `secretref-managed`)로부터 새로 고쳐집니다.
- SecretRef-managed 프로바이더 header 값은 source marker(환경 ref는 `secretref-env:ENV_VAR_NAME`, file/exec ref는 `secretref-managed`)로부터 새로 고쳐집니다.

Marker 기반 저장은 source-authoritative입니다. OpenClaw는 해석된 런타임 비밀 값이 아니라, 활성 source config snapshot(pre-resolution)에서 marker를 기록합니다.
이는 `openclaw agent` 같은 명령 경로를 포함해 OpenClaw가 `models.json`을 재생성할 때마다 적용됩니다.
