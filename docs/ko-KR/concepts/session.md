...
- `per-peer`: `agent:<agentId>:direct:<peerId>` 형태로 격리합니다.
- `per-channel-peer`: `agent:<agentId>:<channel>:direct:<peerId>` 형태로 격리합니다 (다중 사용자 수신함에 권장됨).
- `per-account-channel-peer`: `agent:<agentId>:<channel>:<accountId>:direct:<peerId>` 형태로 격리합니다 (`accountId` 기본값은 `default`, 다중 계정 수신함에 권장됨).
...
- 다이렉트 챗은 `session.dmScope` (기본 `main`)을 따릅니다.
  ...
  - `per-account-channel-peer`: `agent:<agentId>:<channel>:<accountId>:direct:<peerId>` (accountId 기본값은 `default`).
...
- `/status`를 채팅에서 독립된 메시지로 보내 에이전트가 도달 가능한지, 세션 컨텍스트가 얼마나 사용 중인지, 현재 thinking/fast/verbose 토글, 그리고 WhatsApp 웹 인증 정보가 마지막으로 새로고침된 시기를 확인할 수 있습니다 (재링크 필요를 발견하는 데 도움됩니다).
