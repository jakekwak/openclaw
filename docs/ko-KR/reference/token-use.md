...
전체 설명은 [시스템 프롬프트](/concepts/system-prompt)에서 확인하세요.
...
실제 구성 요소 (주입된 파일, 도구, 스킬, 시스템 프롬프트 크기별) 분석은 `/context list` 또는 `/context detail`을 사용하세요. [컨텍스트](/concepts/context)를 참조하세요.
...
[게이트웨이 구성](/gateway/configuration)에서 설정하고 [세션 가지치기](/concepts/session-pruning)에서 행동 세부사항을 참조하세요.
...
정확한 스킬 목록 오버헤드 공식은 [스킬](/tools/skills)에서 확인하세요.
```