...
`openclaw configure --section web`로도 설정할 수 있습니다. 이 값은 `~/.openclaw/openclaw.json`의 `tools.web.search.perplexity.apiKey`에 저장되며, 해당 필드는 SecretRef 객체도 허용합니다.
...
`provider: "perplexity"`가 구성되어 있고 Perplexity 키 SecretRef가 env fallback 없이 해석되지 않으면, startup/reload는 즉시 실패합니다.
