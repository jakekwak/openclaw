...
- 읽기 전용 상태 표면(`status`, `status --json`, `status --all`)은 가능한 경우 대상 설정 경로에 대해 지원되는 SecretRef를 해석합니다.
- 지원 채널 SecretRef가 구성돼 있지만 현재 명령 경로에서 사용할 수 없는 경우에도 상태 명령은 읽기 전용을 유지하며 크래시하지 않고 degraded 출력으로 보고합니다. 사람이 읽는 출력에는 “configured token unavailable in this command path” 같은 경고가 나타나고, JSON 출력에는 `secretDiagnostics`가 포함됩니다.
