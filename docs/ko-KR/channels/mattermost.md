...
`channels.mattermost.replyToMode`를 사용하면 채널 및 그룹 응답이 메인 채널에 남을지, 아니면 트리거한 게시물 아래에서 스레드를 시작할지를 제어할 수 있습니다.

- `off` (기본값): 인바운드 게시물이 이미 스레드 안에 있을 때만 스레드로 응답합니다.
- `first`: 최상위 채널/그룹 게시물에 대해서는 해당 게시물 아래에 스레드를 시작하고, 대화를 스레드 범위 세션으로 라우팅합니다.
- `all`: 현재 Mattermost에서는 `first`와 같은 동작을 합니다.
...
- Mattermost에서 스레드 루트가 한 번 생기면 후속 청크와 미디어가 같은 스레드에 계속 이어지므로, 현재 `first`와 `all`은 동등합니다.
...
접두사가 없는 불투명 ID(예: `64ifufp...`)는 Mattermost에서 **모호합니다**. 사용자 ID일 수도 있고 채널 ID일 수도 있습니다.

OpenClaw는 이를 **사용자 우선**으로 해석합니다:

- 해당 ID가 사용자로 존재하면 (`GET /api/v4/users/<id>` 성공), OpenClaw는 `/api/v4/channels/direct`를 통해 DM 채널을 해결한 뒤 **다이렉트 메시지**를 전송합니다.
- 그렇지 않으면 해당 ID를 **채널 ID**로 취급합니다.

동작을 확정적으로 만들려면 항상 명시적 접두사(`user:<id>` / `channel:<id>`)를 사용하세요.
