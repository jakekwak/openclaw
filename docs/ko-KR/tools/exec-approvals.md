...
- 승인된 node-host 실행은 정규 실행 컨텍스트도 함께 고정합니다. 즉 canonical cwd, 해당될 경우 고정된 executable path, interpreter-style script operand까지 묶습니다. 승인 후 실행 전에 바인딩된 스크립트가 바뀌면, drift된 내용을 실행하지 않고 거부합니다.
