...
- `/mesh <goal>` (자동 계획 및 워크플로 실행; `/mesh plan|run|status|retry`, 동일한 채팅에서 정확한 계획을 재실행하기 위한 `/mesh run <mesh-plan-id>` 포함)
...
- `/subagents list|kill|log|info|send|steer` (현재 세션의 서브-에이전트 실행을 검사, 종료, 로그, 전송 또는 방향 제어)
