...
- 브라우저 tool에서 `profile`을 생략하면 기본 안전값은 격리된 `openclaw` 브라우저입니다.
- `profile="user"`는 실제 로그인 세션, `profile="chrome-relay"`는 확장 attach 흐름용입니다.
- `user`와 `chrome-relay`는 host 전용이며 sandbox/node target과 같이 쓰지 않습니다.
