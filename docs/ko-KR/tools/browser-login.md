...
에이전트 브라우저 도구 호출용 권장 사항:

- 기본 선택: 에이전트는 격리된 `openclaw` 브라우저를 사용해야 합니다.
- `profile="user"`는 기존 로그인 세션이 중요하고, attach 프롬프트를 클릭/승인할 사용자가 컴퓨터 앞에 있을 때만 사용하세요.
- `profile="chrome-relay"`는 Chrome 확장 프로그램 / 툴바 버튼 attach 흐름에만 사용하세요.
