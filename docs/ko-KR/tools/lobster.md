...
- **승인/재개는 내장됨**: 일반 프로그램도 인간에게 프롬프트를 줄 수는 있지만, 내구성 있는 토큰으로 _중지 및 재개_할 수는 없습니다. 이를 위해선 스스로 런타임을 발명해야 합니다.
...
- `lobsterPath`: Lobster 바이너리의 절대 경로 (`PATH` 사용을 원할 경우 생략).
- `cwd`: 파이프라인의 작업 디렉터리 (기본값은 현재 프로세스의 작업 디렉터리).
...
- **강화됨** — 지정된 경우 `lobsterPath`는 절대 경로여야 하며, 타임아웃과 출력 한도가 강제됩니다.
